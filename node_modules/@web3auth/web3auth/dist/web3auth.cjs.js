...
/* harmony import */ var _toruslabs_openlogin__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(1);
...
const getAdapterSocialLogins = function (adapterName, adapter) {
  ...
  if (adapterName === base_["WALLET_ADAPTERS"].CUSTOM_AUTH) {
    var _customAuthAdapter$lo;

    const customAuthAdapter = adapter;
    Object.keys((_customAuthAdapter$lo = customAuthAdapter.loginSettings) === null || _customAuthAdapter$lo === void 0 ? void 0 : _customAuthAdapter$lo.loginProviderConfig).forEach(loginMethod => {
  ...
  } else if (adapterName === base_["WALLET_ADAPTERS"].OPENLOGIN) {
...
class modalManager_Web3Auth extends core_["Web3AuthCore"] {
  constructor(options) {
    ...
    defineProperty_default()(this, "defaultModalConfig", config["b" /* defaultEvmDappModalConfig */]);
    ...
    if (providedChainConfig.chainNamespace === base_["CHAIN_NAMESPACES"].SOLANA) {
      if (options.authMode === "WALLET") {
        ...
        this.defaultModalConfig = config["e" /* defaultSolanaWalletModalConfig */];
      } else {
        // default config for solana dapp modal
        this.defaultModalConfig = config["d" /* defaultSolanaDappModalConfig */];
      }
    } else if (providedChainConfig.chainNamespace === base_["CHAIN_NAMESPACES"].EIP155) {
      if (options.authMode === "WALLET") {
        // default config for evm wallet modal
        this.defaultModalConfig = config["c" /* defaultEvmWalletModalConfig */];
      } else {
        // default config for evm dapp modal
        this.defaultModalConfig = config["b" /* defaultEvmDappModalConfig */];
      }
    } else {
    ...
    this.loginModal = new ui_default.a({
      ...
      loginMethodsOrder: ((_this$options$uiConfi3 = this.options.uiConfig) === null || _this$options$uiConfi3 === void 0 ? void 0 : _this$options$uiConfi3.loginMethodsOrder) || config["a" /* OPENLOGIN_PROVIDERS */]
    });
  ...
  async initModal(params) {
    ...
    const adapterConfigurationPromises = allAdapters.map(async adapterName => {
      var _this$defaultModalCon, _params$modalConfig, _this$defaultModalCon2, _this$defaultModalCon3;

      // start with the default config of adapter.
      let adapterConfig = ((_this$defaultModalCon = this.defaultModalConfig.adapters) === null || _this$defaultModalCon === void 0 ? void 0 : _this$defaultModalCon[adapterName]) || {
        label: adapterName,
        showOnModal: true,
        showOnMobile: true,
        showOnDesktop: true
      ...
      this.defaultModalConfig.adapters[adapterName] = adapterConfig; // check if adapter is configured/added by user and exist in walletAdapters map.

      const adapter = this.walletAdapters[adapterName];
      external_loglevel_default.a.debug("adapter config", adapterName, (_this$defaultModalCon2 = this.defaultModalConfig.adapters) === null || _this$defaultModalCon2 === void 0 ? void 0 : _this$defaultModalCon2[adapterName].showOnModal); // if adapter is not custom configured then check if it is available in default adapters.
      // and if adapter is not hidden by user

      if (!adapter && (_this$defaultModalCon3 = this.defaultModalConfig.adapters) !== null && _this$defaultModalCon3 !== void 0 && _this$defaultModalCon3[adapterName].showOnModal) {
        // if custom auth is configured then no need to use default openlogin
        if (this.walletAdapters[base_["WALLET_ADAPTERS"].CUSTOM_AUTH] && adapterName === base_["WALLET_ADAPTERS"].OPENLOGIN) {
          return;
        ...
        var _this$defaultModalCon4;

        if (!((_this$defaultModalCon4 = this.defaultModalConfig.adapters) !== null && _this$defaultModalCon4 !== void 0 && _this$defaultModalCon4[adapterName].showOnModal)) return; // add client id to openlogin adapter, same web3auth client id can be used in openlogin.
    ...
    const hasInAppWallets = Object.values(this.walletAdapters).some(adapter => adapter.type === base_["ADAPTER_CATEGORY"].IN_APP); // Now, initialize the adapters.
    ...
    const hasExternalWallets = allAdapters.some(adapterName => {
      var _this$walletAdapters$, _this$defaultModalCon5;

      return ((_this$walletAdapters$ = this.walletAdapters[adapterName]) === null || _this$walletAdapters$ === void 0 ? void 0 : _this$walletAdapters$.type) === base_["ADAPTER_CATEGORY"].EXTERNAL && ((_this$defaultModalCon5 = this.defaultModalConfig.adapters) === null || _this$defaultModalCon5 === void 0 ? void 0 : _this$defaultModalCon5[adapterName].showOnModal);
    });
  ...
  async connect() {
    if (!this.loginModal.initialized) throw new Error("Login modal is not initialized"); // if already connected return provider
  ...
  async initExternalWalletAdapters(externalWalletsInitialized, options) {
    ...
    this.loginModal.addWalletLogins(finalAdaptersConfig, adaptersData, {
  ...
  initializeInAppWallet(adapterName) {
    ...
    if (this.walletAdapters[adapterName].type === base_["ADAPTER_CATEGORY"].IN_APP) {
      ...
      this.loginModal.addSocialLogins(adapterName, this.defaultModalConfig.adapters[adapterName], getAdapterSocialLogins(adapterName, this.walletAdapters[adapterName], (_adapterName = this.defaultModalConfig.adapters[adapterName]) === null || _adapterName === void 0 ? void 0 : _adapterName.loginMethods));
    }
  }
