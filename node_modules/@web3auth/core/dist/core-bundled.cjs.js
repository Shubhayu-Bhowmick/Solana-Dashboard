...
class core_Web3AuthCore extends openlogin_jrpc_["SafeEventEmitter"] {
  ...
  configureAdapter(adapter) {
    ...
    if (this.status === base_["ADAPTER_STATUS"].READY) throw base_["WalletInitializationError"].notReady("Adapter is already initialized, so no more adapters can be added");
    const providedChainConfig = this.coreOptions.chainConfig;
    if (!providedChainConfig.chainNamespace) throw base_["WalletInitializationError"].invalidParams("Please provide chainNamespace in chainConfig");

    if (this.walletAdapters[base_["WALLET_ADAPTERS"].OPENLOGIN] && adapter.name === base_["WALLET_ADAPTERS"].CUSTOM_AUTH) {
      throw new Error("Either ".concat(base_["WALLET_ADAPTERS"].OPENLOGIN, " or ").concat(base_["WALLET_ADAPTERS"].CUSTOM_AUTH, " can be used, ").concat(base_["WALLET_ADAPTERS"].OPENLOGIN, " adapter already exists."));
    }

    if (this.walletAdapters[base_["WALLET_ADAPTERS"].CUSTOM_AUTH] && adapter.name === base_["WALLET_ADAPTERS"].OPENLOGIN) {
      throw new Error("Either ".concat(base_["WALLET_ADAPTERS"].OPENLOGIN, " or ").concat(base_["WALLET_ADAPTERS"].CUSTOM_AUTH, " can be used, ").concat(base_["WALLET_ADAPTERS"].CUSTOM_AUTH, " adapter already exists."));
    }
...
/***/ })
/******/ ]);
//# sourceMappingURL=core-bundled.cjs.js.map