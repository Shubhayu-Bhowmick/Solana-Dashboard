    ...
    if (this.uxMode === UX_MODE.REDIRECT) {
      ...
      return new Promise((resolve, reject) => {
        ...
        if (!this.redirectToOpener) {
          bc = new BroadcastChannel("redirect_channel_".concat(this.nonce), broadcastChannelOptions);
  ...
  async getUserInfo(params) {
    ...
    if (accessToken) {
      try {
        ...
        const userInfo = await get("".concat(padUrlString(domainUrl)).concat(user_info_route), {
          headers: {
            Authorization: "Bearer ".concat(accessToken)
          }
        });
...
class JwtHandler extends AbstractLoginHandler {
  ...
  async getUserInfo(params) {
    ...
    try {
      const domainUrl = new URL(domain);
      const userInfo = await get("".concat(padUrlString(domainUrl), "userinfo"), {
        headers: {
          Authorization: "Bearer ".concat(accessToken)
        }
      });
  ...
  handleLoginWindow() {
    return new Promise((resolve, reject) => {
      ...
      const bc = new BroadcastChannel("redirect_channel_".concat(this.nonce), broadcastChannelOptions);
      bc.addEventListener("message", async ev => {
        handleData(ev);
        bc.close();
      });
    ...
    if (extraParamsPassed === "true") {
      ...
      try {
        ...
        ({
          ...
          rpOrigin,
          credId
        } = await get("".concat(WEBAUTHN_LOOKUP_SERVER, "/signature/fetch/").concat(idToken)));
      }
    } else {
...
class CustomAuth {
  ...
  async triggerLogin(args) {
    ...
    if (registerOnly) {
      ...
      return _objectSpread(_objectSpread({}, res), torusKey);
    }
  ...
  async getRedirectResult() {
    ...
    const _retrieveLoginDetails = retrieveLoginDetails(this.config.redirectParamsStorageMethod, instanceId),
      ...
      method
          ...
          rest = _objectWithoutProperties(_retrieveLoginDetails, _excluded4);
...
export { AGGREGATE_VERIFIER, CONTRACT_MAP, LOGIN, NETWORK_MAP, REDIRECT_PARAMS_STORAGE_METHOD, SIGNER_MAP, TORUS_METHOD, TORUS_NETWORK, UX_MODE, broadcastChannelOptions, clearLoginDetailsStorage, clearOrphanedLoginDetails, constructURL, createHandler, CustomAuth as default, eventToPromise, getPopupFeatures, getVerifierId, handleRedirectParameters, isFirefox, loginToConnectionMap, padUrlString, randomId, retrieveLoginDetails, storageAvailable, storeLoginDetails };
